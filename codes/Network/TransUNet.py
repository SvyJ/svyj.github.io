...
def get_r50_b16_config():
    ...
    config.resnet.num_layers = (3, 4, 8)
...
class ResNetV2(nn.Module):
    ...
    def __init__(self, block_units, width_factor):
        ...
        self.root = nn.Sequential(OrderedDict([
            ...
            ('conv', nn.Conv2d(3, width, kernel_size=7, stride=2, bias=False, padding=3)),
...
ACT2FN = {"relu": torch.nn.functional.relu, "swish": swish}
...
class Embeddings(nn.Module):
    ...
    def __init__(self, config, img_size, in_channels=3):
        ...
        self.patch_embeddings = Conv2d(in_channels=in_channels,
                                       ...
                                       kernel_size=1,
                                       stride=1)
        self.position_embeddings = nn.Parameter(torch.zeros(1, img_size[0]//16 * img_size[0]//16, config.hidden_size))  # 这里要改成img_size//16
...
class Encoder(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        for layer_block in self.layer:
            ...
            # # print("Encoder Hidden States:", hidden_states.shape)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        # print("X:", x.shape)
        if x.size()[1] == 1: # 1 channel to 3 channels
            x = x.repeat(1,3,1,1)
        # print("X:", x.shape)
...
# net = VisionTransformer(config=CONFIGS['R50-ViT-B_16'], img_size=512, num_classes=1).to(device)
# # print(net)
