...
class AxialAttention(nn.Module):
    def __init__(self, in_planes, out_planes, groups=8, kernel_size=256, stride=1, bias=False, width=False):
    ...
    def forward(self, x):
        ...
        # stacked_similarity = self.bn_qr(qr) + self.bn_kr(kr) + self.bn_qk(qk) # (N, groups, H, H, W)
...
class AxialAttention_dynamic(nn.Module):
    ...
    def forward(self, x):
        # print(x.shape)
        if self.width:
            x = x.permute(0, 2, 1, 3)  # [N, C, H, W] -> [N, H, C, W]  eg. 2*16*256*256 -> 2*256*16*256
        else:
            x = x.permute(0, 3, 1, 2)  # [N, C, H, W] -> [N, W, C, H]  eg. 2*16*256*256 -> 2*256*16*256 
        # print(x.shape)
        N, W, C, H = x.shape
        x = x.contiguous().view(N * W, C, H)      # eg. (2*256)*16*256

        # Transformations
        qkv = self.bn_qkv(self.qkv_transform(x))  # eg. (2*256)*32*256
        # print("qkv:", qkv.shape)
        q, k, v = torch.split(qkv.reshape(N * W, self.groups, self.group_planes * 2, H), [self.group_planes // 2, self.group_planes // 2, self.group_planes], dim=2)

        # Calculate position embedding
        # print(self.in_planes, self.out_planes, self.groups, self.group_planes)
        # print("relative:", self.relative.shape, "flatten_index:", self.flatten_index, len(self.flatten_index))
        all_embeddings = torch.index_select(self.relative, 1, self.flatten_index).view(self.group_planes * 2, self.kernel_size, self.kernel_size)
        q_embedding, k_embedding, v_embedding = torch.split(all_embeddings, [self.group_planes // 2, self.group_planes // 2, self.group_planes], dim=0)
        # print(q.shape, q_embedding.shape, k.shape, k_embedding.shape)
        qr = torch.einsum('bgci,cij->bgij', q, q_embedding)
        kr = torch.einsum('bgci,cij->bgij', k, k_embedding).transpose(2, 3)
        qk = torch.einsum('bgci, bgcj->bgij', q, k)
        # qr = torch.matmul(q, q_embedding)
        # kr = torch.matmul(k, k_embedding).transpose(2, 3)
        # qk = torch.matmul(q, k)
        ...
        sv = torch.einsum('bgij,bgcj->bgci', similarity, v)
        sve = torch.einsum('bgij,cij->bgci', similarity, v_embedding)
        # sv = torch.matmul(similarity, v)
        # sve = torch.matmul(similarity, v_embedding)
...
class AxialAttention_wopos(nn.Module):
    def __init__(self, in_planes, out_planes, groups=8, kernel_size=256, stride=1, bias=False, width=False):
        ...
        # Multi-head self attention
        self.qkv_transform = qkv_transform(in_planes, out_planes * 2, kernel_size=1, stride=1, padding=0, bias=False) # Conv1d
        self.bn_qkv = nn.BatchNorm1d(out_planes * 2)
...
class AxialBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1, base_width=64, dilation=1, norm_layer=None, kernel_size=256):
...
class ResAxialAttentionUNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1, zero_init_residual=True, groups=8, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None, s=0.125, img_size=304, imgchan = 3):
        ...
        self.decoder2 = nn.Conv2d(int(1024*s), int(1024*s), kernel_size=3, stride=2, padding=1)
    ...
    def _make_layer(self, block, planes, blocks, kernel_size=256, stride=1, dilate=False):
    ...
    def _forward_impl(self, x):
        ...
        # x = F.relu(self.up(self.decoder1(x4)))
        # print(x.shape, x4.shape)
        # x = torch.add(x, x4)
        x = F.relu(self.up(self.decoder2(x3)))
        x = torch.add(x, x3)
        x = F.relu(self.up(self.decoder3(x)))
        x = torch.add(x, x2)
        x = F.relu(self.up(self.decoder4(x)))
        x = torch.add(x, x1)
        x = F.relu(self.up(self.decoder5(x)))
...
class medt_net(nn.Module):
    ...
    def __init__(self, block, block_2, layers, num_classes=1, zero_init_residual=True, groups=8, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None, s=1, img_size=304, imgchan=3):
        super(medt_net, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        self._norm_layer = norm_layer
        self.img_size = img_size # 0322 22:35 js添加
        self.inplanes = int(64 * s)
        self.dilation = 1
        if replace_stride_with_dilation is None:
            replace_stride_with_dilation = [False, False, False]
        ...
        self.layer2_p = self._make_layer(
            block_2, int(256*s),  layers[1], stride=2, kernel_size=(img_size_p//2), dilate=replace_stride_with_dilation[0])
        # self.layer3_p = self._make_layer(
            # block_2, int(512*s),  layers[2], stride=2, kernel_size=(img_size_p//4), dilate=replace_stride_with_dilation[1])
        # self.layer4_p = self._make_layer(
            # block_2, int(1024*s), layers[3], stride=2, kernel_size=(img_size_p//8), dilate=replace_stride_with_dilation[2])
        
        # Decoder
        # self.decoder1_p = nn.Conv2d(int(1024*2*s), int(1024*2*s), kernel_size=3, stride=2, padding=1)
        # self.decoder2_p = nn.Conv2d(int(512*2*s), int(512*2*s),   kernel_size=3, stride=1, padding=1)
        # self.decoder3_p = nn.Conv2d(int(256*2*s),   int(256*2*s),    kernel_size=3, stride=1, padding=1)
        self.decoder4_p = nn.Conv2d(int(256*2*s),    int(256*s),    kernel_size=3, stride=1, padding=1)
    ...
    def _make_layer(self, block, planes, blocks, kernel_size=256, stride=1, dilate=False):
        ...
        layers = []
        layers.append(
            block(self.inplanes, planes, stride, downsample, groups=self.groups, base_width=self.base_width, dilation=previous_dilation, norm_layer=norm_layer, kernel_size=kernel_size))
        self.inplanes = planes * block.expansion
        if stride != 1:
            kernel_size = kernel_size // 2
        ...
        for _ in range(1, blocks):
            layers.append(
                block(self.inplanes, planes, groups=self.groups, base_width=self.base_width, dilation=self.dilation, norm_layer=norm_layer, kernel_size=kernel_size))
    ...
    def _forward_impl(self, x):
        ...
        for i in range(0,4):
            for j in range(0,4):
                ...
                x_p = xin[:, :, (self.img_size//4)*i:(self.img_size//4)*(i+1), (self.img_size//4)*j:(self.img_size//4)*(j+1)]
                # begin patch wise
                ...
                # print(x_p.shape, x3_p.shape)
                # x_p = torch.add(x_p, x3_p)
                # x_p = F.relu(F.interpolate(self.decoder3_p(x2_p) , scale_factor=(2,2), mode ='bilinear'))
                # x_p = torch.add(x_p, x2_p)
                x_p = F.relu(F.interpolate(self.decoder4_p(x2_p) , scale_factor=(2,2), mode ='bilinear'))
                x_p = torch.add(x_p, x1_p)
                x_p = F.relu(F.interpolate(self.decoder5_p(x_p) , scale_factor=(2,2), mode ='bilinear'))
                
                x_loc[:,:,(self.img_size//4)*i:(self.img_size//4)*(i+1),(self.img_size//4)*j:(self.img_size//4)*(j+1)] = x_p
...
def MedT(pretrained=False, img_size=304, **kwargs):
    model = medt_net(AxialBlock_dynamic, AxialBlock_wopos, [1, 2, 4, 1], s= 0.125, img_size=img_size,  **kwargs)
    return model

def logo(pretrained=False, img_size=304, **kwargs):
    model = medt_net(AxialBlock, AxialBlock, [1, 2, 4, 1], s= 0.125, img_size=img_size, **kwargs)
    return model
...
# # net = net.to(device)
# # net = axialunet().to(device) # 304 bingo
# # net = gated().to(device)  # 304 bingo
# # net = MedT(img_size=304).to(device)
# net = logo(img_size=304).to(device)
# print(net)
# summary(net, input_size=(3, 304, 304))