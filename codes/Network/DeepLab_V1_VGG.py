...
def conv3x3_relu(inplanes, planes, rate=1):
    conv3x3_relu = nn.Sequential(
        nn.Conv2d(inplanes, planes, kernel_size=3, stride=1, padding=rate, dilation=rate),
        nn.ReLU()
    )
    return conv3x3_relu
...
class DeepLabv1_ASPP(nn.Module):
    def __init__(self, num_classes, small=True, pretrained=False):
        super(DeepLabv1_ASPP, self).__init__()
...
class DeepLabv1_FOV(nn.Module):
    def __init__(self, num_classes, pretrained=True):
        super(DeepLabv1_FOV, self).__init__()
        self.vgg_feature = VGG16_feature(pretrained)
...
model = DeepLabv1_ASPP(num_classes=1).to(device)
print(model)
summary(model, input_size=(3, 224, 224))    ## 这里还有问题