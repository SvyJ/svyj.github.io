...
class CrossViT(nn.Module):
    def __init__(self, image_size, channels, num_classes, patch_size_small = 14, patch_size_large = 16, small_dim = 96,
                 large_dim = 192, small_depth = 1, large_depth = 1, cross_attn_depth = 1, multi_scale_enc_depth = 1,
...
model = CrossViT(image_size=304, channels=3, num_classes=1, patch_size_small=8, patch_size_large=38).to(device)
print(model)
parameters = filter(lambda p: p.requires_grad, model.parameters())
parameters = sum([np.prod(p.size()) for p in parameters]) / 1_000_000
print('Trainable Parameters: %.3fM' % parameters)
summary(model, input_size=(3, 304, 304))  # [B, num_classes]
